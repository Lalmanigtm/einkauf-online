...
const Navbar = () => {
    ...
    // todo: handle cartCount
    const cartCount = useCart((s: any) => s.items.reduce((n: number, line: any) => n + line.quantity, 0))
            ...
            <div className='navbar mx-auto min-h-14 max-w-7xl px-4 py-2.5 md:px-6 md:py-3'>
                <div className='flex-1'>
                    <Link
                        ...
                        <span className='leading-none'>Einkauf-Online</span>
                    </Link>
                </div>
                ...
                <nav className='flex items-center gap-1 md:gap-1.5'>
                    ...
                    <Link to="/cart" className='btn btn-ghost gap-2 font-medium indicator' aria-label={cartCount > 0 ? `Cart, ${cartCount} items` : "Cart"}>
                        {
                            cartCount > 0 ? (
                                <span className='indicator-item badge badge-sm badge-primary min-w-2 px-1.5 font-sans text-xs tabular-nums'> {cartCount > 99 ? "99+" : cartCount}
